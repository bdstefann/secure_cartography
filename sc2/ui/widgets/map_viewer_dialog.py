...
def _filter_sc2_format(topology: Dict, connected_only: bool, include_leaves: bool) -> Dict:
    ...
    # Post-trim orphan sweep: peer trimming above may have emptied a node's
    # peers while no remaining node references it, leaving it visually
    # disconnected.  When connected_only is active, drop those stranded nodes.
    if connected_only:
        # Rebuild the "referenced by someone in filtered set" index
        still_referenced: Set[str] = set()
        for node_data in filtered.values():
            for peer_name in node_data.get('peers', {}):
                still_referenced.add(peer_name)
                still_referenced.add(peer_name.lower())

        stranded = [
            name for name, data in filtered.items()
            if not data.get('peers')
               and name not in still_referenced
               and name.lower() not in still_referenced
        ]
